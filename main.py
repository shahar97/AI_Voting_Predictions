...
"""
The main.py file in the AI_Voting_Predictions repository is the primary Python script that runs the program. The 
program aims to predict the political preferences of individual voters based on a variety of demographic and other 
factors. 

When the main.py file is executed, it first imports several Python modules, including pandas, numpy, and sklearn. 
These modules are used to manipulate data, perform calculations, and train machine learning models. 

Next, the program reads in a dataset of voter information from a CSV file. This data includes attributes such as age, 
gender, education level, and income, as well as the voter's political party affiliation and the candidate they voted 
for in the previous election. 

The data is then preprocessed, which includes scaling numeric data, encoding categorical data, and splitting the 
dataset into training and testing sets. 

After preprocessing, the program trains several machine learning models using the training data. These models include 
a decision tree classifier, a random forest classifier, and a gradient boosting classifier. 

Once the models are trained, the program evaluates their performance using the testing data. It calculates several 
metrics, including accuracy, precision, recall, and F1 score, and displays the results. 

Finally, the program prompts the user to input their own information, and uses the trained models to predict the 
political party affiliation of the user based on their input. 

Overall, the main.py file serves as the backbone of the AI_Voting_Predictions program, reading in data, preprocessing 
it, training machine learning models, evaluating their performance, and making predictions based on user input. 
"""
